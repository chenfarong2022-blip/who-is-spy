...
function showLobbyDemo() {
  ...
  document.getElementById('room-id-display').textContent = roomId || 'DEMO';
  document.getElementById('total-players').textContent = playerCount;
  
  // 生成二维码
  const roomUrl = window.location.origin + window.location.pathname.replace('index.html', 'player.html') + '?room=' + (roomId || 'demo');
